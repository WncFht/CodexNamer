...
import { autoRenameReasonLabel, autoRenameStatusLabel, formatUiWhen, type UiLanguage } from "./i18n.js";
...
function Surface(props: {
  ...
  return (
    <Box flexDirection="column" width={props.width} height={props.height}>
      ...
      <Box borderStyle="round" borderColor={TONE.border} flexDirection="column" paddingX={1} width={props.width} height={props.height ? Math.max(4, props.height - 1) : undefined} overflow="hidden">
        {props.children}
      </Box>
    </Box>
  );
...
export function MaintenanceScreen(props: {
  ...
  const skipSummary = [...skipReasons.entries()].sort((left, right) => right[1] - left[1]).slice(0, 6);
  const logItems = props.aiRequestLogs?.items.slice(0, Math.max(5, props.layout.visiblePreviewCount + 2)) ?? [];
      ...
      <Box flexDirection={props.layout.stacked ? "column" : "row"} gap={1}>
        <Box flexDirection="column" width={topWidth} gap={1}>
          <Surface
            ...
            subtitle={inline(props.uiLanguage, "即时评估，不是 daemon 快照", "live preview, not daemon snapshot")}
            width={topWidth}
          >
            {props.preview.length === 0 ? (
              <Text color={TONE.muted}>{inline(props.uiLanguage, "还没有加载预览。", "No preview loaded.")}</Text>
            ) : null}
            {props.preview.slice(0, Math.max(6, props.layout.visibleSessionCount)).map((item, index) => (
              <Text color={item.status === "apply" ? TONE.success : item.status === "suggest" ? TONE.warning : TONE.muted} key={`${item.threadId}-${index}`} wrap="truncate-end">
                {fitLine(
          ...
          >
            ...
            {replayRuns.length === 0 ? (
              <Text color={TONE.muted}>{inline(props.uiLanguage, "还没有 replay 记录。", "No replay runs yet.")}</Text>
            ) : null}
        ...
        <Box flexDirection="column" width={rightWidth} gap={1}>
          ...
          >
            ...
            {logItems.length === 0 ? (
              <Text color={TONE.muted}>{inline(props.uiLanguage, "还没有 AI 请求日志。", "No AI request logs yet.")}</Text>
            ) : null}
            {logItems.map((item) => (
              <Box flexDirection="column" key={item.id} marginBottom={1}>
                <Text color={item.status === "failed" ? TONE.danger : item.status === "running" ? TONE.warning : TONE.success} wrap="truncate-end">
                  {fitLine(
