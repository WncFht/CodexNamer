...
function buildRenamePrompt(session: MaterializedSession, config: EffectiveConfig): string {
  const parts = [
    ...
    `Rename context strategy: ${config.naming.contextStrategy}.`,
    ...
    `renameUserMessagesText: ${normalizePromptField(session.renameUserMessagesText, 2200)}`,
    `renameAssistantMessagesText: ${normalizePromptField(session.renameAssistantMessagesText, 2200)}`,
    `renameContextText: ${normalizePromptField(session.renameContextText, 3200)}`,
