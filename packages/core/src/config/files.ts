...
import { DEFAULT_CONFIG, type DeepPartial } from "./defaults.js";
import {
...
async function loadCodexInheritedConfig(codexHome: string): Promise<EffectiveConfig["inheritedCodex"]> {
  ...
  try {
    ...
    const tokens = authRaw.tokens && typeof authRaw.tokens === "object" ? (authRaw.tokens as Record<string, unknown>) : undefined;
...
export async function loadEffectiveConfig(options?: {
  ...
  const explicitStateDir = options?.overrides?.general?.stateDir ?? projectRaw.general?.stateDir ?? userRaw.general?.stateDir;
  effective.general = {
