...
export function getRenameState(db: Database.Database, threadId: string): RenameStateRecord | undefined {
  const row = db.prepare(`SELECT * FROM rename_state WHERE thread_id = ?`).get(threadId) as Record<string, unknown> | undefined;
...
function getLatestRenameHistoryRow(db: Database.Database, threadId: string): Record<string, unknown> | undefined {
  return db.prepare(
...
export function listNonAcceptedNamedThreadIds(db: Database.Database, acceptedSources: RenameSource[]): Set<string> {
