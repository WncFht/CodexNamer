...
- transcript 过滤规则
- context 截断规则
- summary-signals、transcript 与 paired-turn 策略的语义
...
### 5.3 `paired-user-turns`

这是 transcript 类策略里更强调“任务推进线”的版本。

它同样要求上层提供 transcript，并沿用与 `user-assistant-transcript` 相同的可见性过滤：
...
输出不再是单纯的最近消息堆叠，而是按 user turn 组织：

1. 首条 user 仍作为整体目标保留
2. 对每个后续 user：
   - 只看它前面紧邻的一段 assistant cluster
   - 不会跨过更早的 user 往前回溯
3. 在这段 cluster 里，从后往前找第一条“有信息量的 assistant”
   - 找到就挂成 `assistant(context)`
   - 找不到就只保留当前 `user(turn)`
...
#### “有信息量的 assistant” 当前判定

当前实现不是简单地取最后一条 assistant，而是做一个轻量过滤：

- 先排除占位内容，例如：
  - `内容过长`
  - `content too long`
- 再排除过短且明显是过程话的 assistant，例如：
  - `我先看一下`
  - `我现在开始改`
  - `Let me check`
- 如果文本里出现了结果、定位、修复、确认、失败/成功等强信号词，则直接视为有效
- 否则要求它至少达到一个最小信息密度门槛，例如：
  - 有足够长度
  - 或同时包含标点、路径、代码标记、数字、子系统词等多个信号

这层过滤的目标不是做摘要质量评分，而是避免把纯过程话错挂到 user turn 前面。
...
- `最近一条 assistant 原样拿`
  - 偶尔会吃到“我先改一下”“稍等我检查”这类低信息量过程话
- `同一段里挑最长的 assistant`
  - 更容易把旧计划或更早的大段说明带回来
  - 对 rename 来说会把标题往旧主题拉偏

因此当前默认是：

- 只看前一个 assistant cluster
- 只取这段 cluster 里最后一条 substantive assistant
- 如果没有，就宁可只保留 user

## 6. 回退规则

当请求策略属于 transcript / paired 类时：
...
- `summary-signals / last-user-last-assistant / user-assistant-transcript / user-only-transcript / assistant-only-transcript / user-transcript-last-assistant / paired-user-turns`
  - 这是“上下文取材层”
...
- `requestedContextStrategy = paired-user-turns`
  - prompt 按 user turn 组织上下文
  - 每个 turn 只挂前一段里最后一条有效 assistant
  - 更适合保留“上一轮结论 -> 当前用户推进”的结构
...
其中真正的 user / assistant 内容会放在 fenced code block 里：
