...
这里的 `apply` 只表示“从调度逻辑上允许正式 rename”，不代表 daemon 已经真的落盘。

当前项目的实际现状仍然是：

- daemon 仍然只做 preview
- `previewAutoRename()` 现在复用 `evaluateAutoRename`
...
也就是说，前端现在看到的是统一后的评估结果，不再需要自己二次推断。
...
2. 再把 `previewAutoRename` 和未来 daemon auto-apply 完全建立在这个入口之上
3. 再考虑是否为真正 auto-apply 增加“候选稳定一轮 scan”之类的额外 gate
